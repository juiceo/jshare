...
import { getAutoGeneratedProps, type AutoGeneratedProps } from '~/lib/store/util';
...
> = {
    ...
    api: A;
    /**
     * A function that creates an optimistic document from the input and auto-generated properties.
     *
...
> {
    ...
    index: IndexedMap<DocumentStore<S, A, R, I, T>>;
    resolvers?: R;
    hooks?: DocumentStoreHooks<T>;
    staleTime: number;
    schema: S;
    ...
    constructor(args: DocumentStoreArgs<S, A, R, I, T>) {
        ...
        this.flushQueries = debounce(
            async () => {
                ...
                if (!this.api.findMany) {
                    if (queries.length > 0) {
                        console.warn(
                            `Attempted to call ${this.name}.findMany, but no such API is defined. findMany queries for this collection will be ignored.`
                        );
                        queries = [];
                    }
                }
        ...
        const docs = parsedItems
            .map((item: any) => {
                ...
                try {
                    ...
                    return new Document<DocumentStore<S, A, R, I, T>>(
                        this,
                        parsed as T,
                        this.resolvers
                    );
    ...
    invalidate(queryOrId: string | Query<T>) {
    ...
    private findManyComputed = computedFn(
        (args: string): QueryResultsArray<DocumentStore<S, A, R, I, T>> => {
            ...
            }) as QueryResultsArray<DocumentStore<S, A, R, I, T>>;
        }
    );
    ...
    registerItem(item: T): Document<DocumentStore<S, A, R, I, T>> {
        if (this.index.has(item.id)) {
            this.index.update(item.id, item);
            return this.index.get(item.id)!;
        } else {
            const doc = new Document<DocumentStore<S, A, R, I, T>>(this, item, this.resolvers);
    ...
    ): Promise<Document<DocumentStore<S, A, R, I, T>> | null> {
