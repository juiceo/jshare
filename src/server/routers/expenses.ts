...
import { ExpenseWithSenderAndShares, createExpenseSchema, editExpenseSchema } from '@/schemas/expense';
...
export const expenseRouter = trpc.router({
	create: trpc.authenticatedProcedure.input(createExpenseSchema).mutation(async ({ input, ctx }) => {
		try {
	...
	edit: trpc.authenticatedProcedure.input(editExpenseSchema).mutation(async ({ input, ctx }) => {
		...
		ee.emit(Events.EditExpenseInGroup(expenseAfter.groupId), expenseAfter);
	...
	onCreateExpenseInGroup: trpc.authenticatedProcedure.input(z.string()).subscription(async ({ input }) => {
	...
	onEditExpenseInGroup: trpc.authenticatedProcedure.input(z.string()).subscription(async ({ input }) => {
		return observable<ExpenseWithSenderAndShares>((emit) => {
			const onSend = (data: ExpenseWithSenderAndShares) => {
				emit.next(data);
			};
	...
	listByGroupId: trpc.authenticatedProcedure.input(z.string()).query(async ({ input, ctx }) => {
		const groupId = input;
		const group = await prisma.group.findUnique({
			where: {
				id: groupId,
	...
	getTotalByGroupId: trpc.authenticatedProcedure.input(z.string()).query(async ({ input, ctx }) => {
		const group = await prisma.group.findUnique({
			where: {
				id: input,
			},
